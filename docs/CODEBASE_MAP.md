...
├── style.ts            # CSS exports for rendered output
...
- CSS variables (`--color-*`) for Primer theming
...
- All CSS is minified and manually maintained (no generation pipeline yet)
...
**To modify CSS/theming**: Edit `style.ts` directly (see ROADMAP.md for planned
generation pipeline)
